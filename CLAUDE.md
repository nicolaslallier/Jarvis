...
- `backend/app/routers/` — one `APIRouter` per resource (`health.py`, `items.py`, `tasks.py`, `chat.py`, `files.py`, `ingest_status.py`). `chat.py`'s `send_message` embeds the user's message and calls `app/rag.py`'s `fetch_relevant_chunks()` to fetch the `RAG_TOP_K` nearest `file_chunks`, injecting them as a leading system message sent to the model (not persisted as a `ChatMessageRecord`, so it isn't repeated back into history on the next turn). This is best-effort: any failure (LM Studio unreachable, `vector` extension/table not provisioned yet, no chunks at all) is caught and logged, falling back to a plain chat call — RAG is a quality boost, never a reason a message fails to send. `files.py` uses `jarvis_shared.storage` for all MinIO calls (no inline boto3 client of its own) and exposes `POST /files/{id}/ingest`, which publishes a `jarvis.ingest.requested` message instead of touching Docker directly. `ingest_status.py` exposes the `GET /ws/ingest-status` WebSocket the frontend listens on.
