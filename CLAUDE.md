...
- `frontend/Dockerfile` — multi-stage build: `npm run build` then serves the static `dist/` via nginx. `VITE_API_URL` is a **build-time** arg (baked into the static bundle) since it's a browser-facing URL. It points at the Infra nginx vhost's `/api/` reverse-proxy (`https://jarvis.famillelallier.net/api`, see `nginx/conf.d/jarvis.conf` in the Infra repo — a `location /api/` block proxying to `jarvis-api:8000` on `infra-net`), same-origin with the frontend itself, rather than the backend's published host port directly. This matters because `VITE_API_URL` is resolved by the *browser*, not the Docker host: a value like `http://localhost:8000` only works when the browser and the Docker host are the same machine, and breaks with `ERR_CONNECTION_REFUSED` from any other machine on the LAN (its `localhost` resolves to itself, not the Docker host). Going through the shared nginx vhost instead means every LAN browser reaches it at one stable hostname regardless of which machine runs Docker.
